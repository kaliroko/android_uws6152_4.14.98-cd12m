...
static int cst9217_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!IS_ERR(p)) {
        struct pinctrl_state *s = pinctrl_lookup_state(p, "hyn_irq_gpio");
        if (!IS_ERR(s)) {
            pinctrl_select_state(p, s);
            dev_info(&client->dev, "Set pinctrl to hyn_irq_gpio\n");
        } else {
            dev_warn(&client->dev, "hyn_irq_gpio state not found\n");
        }
    }
    ...
    // GPIO 解析
    if (of_property_read_u32_array(np, "reset-gpio", gpios, 3) == 0)
        ts->reset_gpio = gpios[1];
    else
        ts->reset_gpio = -1;

    if (of_property_read_u32_array(np, "irq-gpio", gpios, 3) == 0)
        ts->irq_gpio = gpios[1];
    else
        ts->irq_gpio = -1;
    ...
    // 分辨率
    if (of_property_read_u32_array(np, "display-coords", coords, 4) == 0) {
        ts->abs_x_max = coords[2];
        ts->abs_y_max = coords[3];
    } else if (of_property_read_u32_array(np, "display-coords", coords, 2) == 0) {
        // 兼容只有两个值的写法
...
static const struct of_device_id cst9217_match[] = { { .compatible = "hynitron,hyn_ts" }, { .compatible = "hyn,92xx" }, { } };
static const struct i2c_device_id cst9217_id[] = { { "hyn_ts", 0 }, { "92xx", 0 }, { } };
